...
from modelos import criar_stats_text, validar_valores, prever_cultura
...
def criar_grafico_distribuicao(fig, df):
    ...
    counts.plot.bar(
        ax=ax1,
        color=["#4e79a7", "#f28e2b", "#e15759", "#76b7b2", "#59a14f", "#edc948", "#b07aa1", "#ff9da7", "#9c755f", "#bab0ab"],
    )
...
def criar_interface(df, modelo, encoder, resultados, importancia_df, valores_ideais, importancias_por_variavel):
    """Cria a interface gráfica principal com abas de recomendação, EDA e comparação de modelos."""
    root = tk.Tk()
...
def criar_tab_recomendacao(parent, modelo, encoder, valores_ideais, importancias_por_variavel):
    ...
    def gerar_explicacao(cultura_top, valores_input):
        """Gera uma explicação baseada no desvio em relação aos valores ideais e na importância das variáveis."""
        ...
        impactos.sort(key=lambda item: item[5], reverse=True)
        top_3 = impactos[:3]
